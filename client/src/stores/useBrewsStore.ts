...
interface BrewsState {
    ...
    addBrew: (b: Omit<Brew, 'id' | 'startedAt' | 'finishedAt' | 'infusions' | 'created_at' | 'updated_at' | 'user_id' | 'deleted_at'> & { presetId?: string }) => void;
...
export const useBrewsStore = create<BrewsState>((set, get) => ({
    ...
    updateBrew: (id, updates) => {
        const now = new Date().toISOString();
        const updated = get().brews.map(b => b.id === id ? { ...b, ...updates, updated_at: now } : b);
        storage.set('brews', updated);
        set({ brews: updated });
        if (navigator.onLine) get().syncBrews();
    },
    ...
    deleteBrew: (id) => {
        const now = new Date().toISOString();
        const updated = get().brews.map(b => b.id === id ? { ...b, deleted_at: now, updated_at: now } : b);
        storage.set('brews', updated);
        set({ brews: updated });
        if (navigator.onLine) get().syncBrews();
    },
    ...
    addInfusion: (brewId, infusion) => {
        ...
        const updated = get().brews.map(b => {
            if (b.id === brewId) {
                ...
                return { ...b, infusions: [...b.infusions, { ...infusion, number: nextNumber }], updated_at: now };
            }
            return b;
        });
        storage.set('brews', updated);
    ...
    addPreset: (p) => {
        ...
        if (!user) return;
        const now = new Date().toISOString();
        const newPreset: Preset = { ...p, id: uuid(), user_id: user.id, created_at: now, updated_at: now, deleted_at: null };
    ...
    updatePreset: (id, updates) => {
        const now = new Date().toISOString();
        const updated = get().presets.map(p => p.id === id ? { ...p, ...updates, updated_at: now } : p);
        storage.set('presets', updated);
        set({ presets: updated });
        if (navigator.onLine) get().syncPresets();
    },
    ...
    deletePreset: (id) => {
        const now = new Date().toISOString();
        const updated = get().presets.map(p => p.id === id ? { ...p, deleted_at: now, updated_at: now } : p);
        storage.set('presets', updated);
        set({ presets: updated });
        if (navigator.onLine) get().syncPresets();
    },
